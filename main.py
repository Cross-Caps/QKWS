...
from Utils import get_data, apply
import train
import Utils.util as util   
from model import QBcResNetModelASM
...
def run(model, train_loader, validation_loader, test_loader, optimizer, scheduler, device, checkpoint_file, n_epoch=10, log_interval=100):
    ...
    for epoch in range(n_epoch):
        ...
        train.train_epoch_asm(model, optimizer, train_loader, device, epoch, log_interval=log_interval)
        if scheduler:
            scheduler.step()
...
def train_command(scale, batch_size, device, epoch, log_interval, checkpoint_file, optimizer, dropout, subspectral_norm):
    ...
    model = QBcResNetModelASM(n_class=get_data.N_CLASS, scale=scale,dropout=0.2,use_subspectral=True).to(device)
...
##########this code is for applying file model to one audio file.
